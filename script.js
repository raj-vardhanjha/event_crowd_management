...
// Create decorative bubbles inside the hero section for a gentle aesthetic motion
function createBubbles(count = 20) {
    ...
    const colors = [
        ...
        'rgba(255,78,140,0.4)', // vibrant pink
        'rgba(255,215,0,0.4)',  // golden yellow
        'rgba(0,250,154,0.4)'   // spring green
    ];

    // Create a fixed number of bubbles to avoid DOM bloat
    for (let i = 0; i < count; i++) {
        const b = document.createElement('div');
        b.className = 'bubble';

        // random size between 20 and 160
        const size = Math.floor(Math.random() * 140) + 20;
        const left = Math.random() * 100; // percent
        const duration = Math.random() * 18 + 10; // 10s - 28s
        const delay = Math.random() * -20; // start at negative delays for spread
        const color = colors[Math.floor(Math.random() * colors.length)];

        b.style.width = `${size}px`;
        b.style.height = `${size}px`;
        b.style.left = `${left}%`;
        b.style.background = `radial-gradient(circle at 35% 35%, rgba(255,255,255,0.9), ${color})`;
        b.style.animationDuration = `${duration}s`;
        b.style.animationDelay = `${delay}s`;
        b.style.opacity = `${0.55 + Math.random() * 0.3}`;
